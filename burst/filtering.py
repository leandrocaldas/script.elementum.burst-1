...
def cleanup_results(results_list, process_sizes = False):
    ...
    dict_hash_size = {} # burst: if some provider got size, we copy it to the others
    #results_list = sorted(results_list, key=lambda r: len(r['size']), reverse=True) # burst: assert results with size come first
    ...
    for result in results_list:
        ...
        # burst: pass size to all results with same hash
        ...
        if not any(existing == hash_ for existing in hashes):
            if not show_providers_name and "FFF14E13" in result['provider']: # burst: remove provider name if brazilian (check by name color at json file)
